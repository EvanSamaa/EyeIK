...
class Andrist_SacccadeGenerator:
    # helper functions
    ...
    def get_head_velocity(self, g:float):
        v = 0
        if g <= 0.5:
            v = 2 * g
        else:
    ...
    def add_head_submovement(self, t0, p0_not_normalized, p1_not_normalized):
        ...
        # angle in degrees
        rot_angle = rot_angle * 180 / np.pi
        rot_angle = rot_angle * self.head_propensity
        if self.head_propensity == 0:
            return None, None
        angle = 0
        ...
        # if force_stop_step != []:
        #     forced_stop_length = force_stop_step[1] - force_stop_step[0]
    ...
    def compute(self):
        ...
        while np.ceil(self.t) < end_t:
            ...
            if abs(saccade_generation_test - int(round(saccade_generation_test))) <= 0.001:
                ...
                # for Andrist, since head movement only occurs during eye movement, we only move head when eye is moved 
                    head_submovement, head_submovement_range = self.add_head_submovement(self.t + self.head_latency, self.head_current_goal_position, self.gaze_current_goal_position)
