...
class Itti_SacccadeGenerator:
    ...
    def get_head_movement_duration(self, pos0, pos1):
        return 0.4
    def head_max_velocity(self, H):
        Hv = 13 * H + 16
        return Hv
    def eye_max_velocity(self, E):
        Ev = 473 * (1 - np.exp(-E/7.8))
        return Ev
    ...
    def compute(self):
        ...
        while np.ceil(self.t) < end_t:
            ...
            if abs(saccade_generation_test - int(round(saccade_generation_test))) <= 0.001:
                ...
                iep = -1 * (np.sign(goal_gaze_angle * current_gaze_angle)) # if they are in the same direction then it is negative
                iep = iep * np.sign(current_gaze_angle)
                T = (-iep / 2 + 20) * 0.56
                if -T < H_angle and H_angle < T:
                    new_H_angle = 0
                else:
                    k = (iep / 35 + 1.1) * 0.65
                    new_H_angle = (1-k) * T + k * H_angle
                ...
                head_movement_duration = self.get_head_movement_duration(self.head_current_goal_position, new_head_goal)
                # add the head movement
                head_submovement, head_submovement_range = self.add_head_submovement(self.t , self.t + head_movement_duration , self.head_current_goal_position, new_head_goal)
