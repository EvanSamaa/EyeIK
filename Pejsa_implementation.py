...
class Pejsa_SacccadeGenerator:
    ...
    def interpolate_goal(self, t):
        ...
        print("Error")
    def __init__(self, target_times, target_positions, target_index, internal_model, head_propensity=0):
    ...
    def get_head_movement_maximum_speed(self, pos0, pos1):
        p0 = pos0/np.linalg.norm(pos0) * 100
        p1 = pos1/np.linalg.norm(pos1) * 100
        rot_axis, rot_angle = rotation_axis_angle_from_vector(p0, p1)
        rot_angle = np.abs(rot_angle)
        rot_angle = rot_angle * 180 / np.pi
        maximum_speed = 4.39 * rot_angle + 106. 
        
        return maximum_speed
    def get_head_velocity(self, r:float):
        v = 0
        if r <= 0.5:
            v = 1.5 * r + 0.25
        else:
    ...
    def add_head_submovement(self, t0, p0_not_normalized, p1_not_normalized):
        ...
        submovement_speed = [] # to store the speed of all points in the submovements
        submovement_directions = [] # to store the position of all points in the submovements
        t1 = t0
        vel = 0
        # the the scalar angles based on the velocity profile
        r = 0        
        while np.abs(rot_angle - angle) > 0.01:
            r = r + self.simulation_dt * vel / (rot_angle)
            vel = self.get_head_velocity(r) * self.get_head_movement_maximum_speed(p0, p1)
            if vel == 0 or r >= 1:
                break
            angle += vel * self.simulation_dt
            submovement_postion += [angle]
            t1 += self.simulation_dt   
        ...
        for i in range(len(submovement_postion)):
            submovement_postion[i] = submovement_postion[i] * np.pi / 180
            rot_matrix = np.eye(3) + np.sin(submovement_postion[i]) * skew_symmetric_matrix_rot_axis + (1 - np.cos(submovement_postion[i])) * skew_symmetric_matrix_rot_axis @ skew_symmetric_matrix_rot_axis
            submovement_direction = (rot_matrix) @ (p0)
            submovement_directions.append(submovement_direction) 
        submovement_directions = np.array(submovement_directions) # should be of shape N by 3
        ...
        while np.ceil(self.t) < end_t:
            ...
            if abs(saccade_generation_test - int(round(saccade_generation_test))) <= 0.001:
                ...
                if not gaze_submovement is None:
                    ...
                    # compute the 
                    Hrot_axis, H_angle_max = rotation_axis_angle_from_vector(self.head_current_goal_position, self.gaze_current_goal_position)
                    H_angle_max *= 180 / np.pi
                    # get the horizontal angle of the gaze
                    goal_gaze_angle = rotation_angles_frome_positions(self.gaze_current_goal_position)[0]
                    current_gaze_angle = rotation_angles_frome_positions(self.gaze_positions[t_index])[0]
                    if goal_gaze_angle * current_gaze_angle > 0:
                        iep = 0
                    else:
                        iep = goal_gaze_angle
                    omr = 45 * (1/360 * iep + 0.75)
                    H_angle_min = np.maximum(0, H_angle_max - omr)
                    new_H_angle = self.head_propensity * H_angle_max + (1 - self.head_propensity) * H_angle_min
                    new_H_angle *= np.pi / 180
                    new_head_goal = rotation_matrix_from_axis_angle(Hrot_axis, new_H_angle) @ self.head_current_goal_position
                    head_submovement, head_submovement_range = self.add_head_submovement(self.t + self.head_latency, self.head_current_goal_position, new_head_goal)
